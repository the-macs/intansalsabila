    ...
    <section class="bg-white">
        <div class="px-4 md:px-6">
            <div class="grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 items-center justify-center">
                ...
                <img src="{{ asset('storage/' . $airline->image_path) }}" alt="{{ basename($airline->name) }}" title="{{ $airline->name }}" class="w-full transition-transform duration-300 group-hover:scale-110 p-14" />
                @endforeach
            </div>
        </div>
    </section>

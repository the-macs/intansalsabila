            ...
            <div class="container px-4 mx-auto md:px-6">
                    ...
                    @foreach ($articles as $article)
                        <x-card class="overflow-hidden hover:shadow-lg transition-shadow duration-300">
                            <div class="relative h-48">
                                <img src="{{ $article['image'] ?? '/images/article-placeholder.jpg' }}"
                                    alt="{{ $article['title'] }}" class="object-cover w-full h-full">
                            </div>
                            <x-card.header class="min-h-40">
                                <x-card.title>{{ $article['title'] }}</x-card.title>
                                <x-card.description>{{ Str::limit(strip_tags($article['content']), 150, '...') }}</x-card.description>
                            </x-card.header>
                            <x-card.content>
                ...
                {{-- <div class="mt-12 flex justify-center">
                    <div class="flex gap-2">
                        ...
                        <button
                            class="inline-flex items-center rounded-md border border-emerald-600 bg-emerald-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-emerald-700">
                            1
                        </button>
                        <button
